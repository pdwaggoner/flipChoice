...
parameters {
    ...
    vector[V_covariates] covariates_beta[P - 1];
}

transformed parameters {
    matrix[V, V] L_sigma[P];
    vector[V] class_beta[R, P];
    vector[P] discriminant[R];
    simplex[P] covariates_class_weights[R];
    ...
    for (r in 1:R)
    {
        discriminant[r, 1] = 0;
        for (p in 2:P)
            discriminant[r, p] = covariates[r] * covariates_beta[p - 1];
        covariates_class_weights[r] = softmax(discriminant[r]);
    }
}
...
generated quantities {
    ...
    // Add braces to exclude rs from exported values
    {
        ...
        for (r in 1:R)
        {
            ...
            for (p in 1:P)
            {
                rlh[r] += covariates_class_weights[r, p] * exp(posterior_prob[p] / S[r]);
                posterior_prob[p] += log(covariates_class_weights[r, p]);
            }
    ...
    if (S_out > 0)
    {
        ...
        for (r in 1:R)
        {
            ...
            for (p in 1:P)
            {
                rlh_out[r] += covariates_class_weights[r, p] * exp(posterior_prob[p] / S_out);
                posterior_prob[p] += log(covariates_class_weights[r, p]);
            }
