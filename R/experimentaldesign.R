...
#' @param none.positions Integer \code{\link{vector}} specifying the
...
#' @param n.constant.attributes Integer; the number of attributes to
#'     keep constant for partial profiles.
#' @param extensive Logical; whether to used the extensive algorithm
#'     instead of the integrated algorithm for partial profiles.
#' @param n.rotations The number of random rotations performed when
...
#' }
#' @references The partial profiles algorithm is an implementation of
...
ChoiceModelDesign <- function(design.algorithm = c("Random", "Shortcut",
    ...
    if (design.algorithm == "Efficient")
        design.algorithm <- "Partial profiles"
